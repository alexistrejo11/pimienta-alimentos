...
class DeviceApiMockWebServerTest {
 ...
 @Test fun bootstrapCursorAndCentavos()=runBlocking{
  ...
  val result=api.bootstrap(); Assert.assertEquals("c1",result.cursors.changes); Assert.assertEquals(1250L,result.products.single().priceCentavos); Assert.assertEquals(10, result.products.single().stockQuantity)
 }
 @Test fun bootstrapPoliciesWithoutDefaultNegativeStockLimit()=runBlocking{
  ...
  server.enqueue(MockResponse().setResponseCode(200).setBody("""{"schemaVersion":1,"kind":"FULL","snapshotId":"s","generatedAt":"2026-01-01T00:00:00Z","site":{"id":"1","name":"Centro","address":"","currency":"MXN"},"device":{"id":"d","name":"POS","visibleCode":"T1","status":"ACTIVE"},"operators":[],"products":[],"openAmountCategories":[],"policies":{"allowNegativeStock":true,"staleCatalogWarnHours":24,"staleCatalogBlockHours":72},"cursors":{"changes":"c1"}}""").addHeader("Content-Type","application/json"))
