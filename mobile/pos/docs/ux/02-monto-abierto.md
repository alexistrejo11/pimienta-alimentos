...
No es un producto maestro ni una forma de editar catálogo desde la tablet. Es una línea excepcional marcada para revisión posterior.
...
No pide PIN ni descripción libre. El POS construye la descripción visible con el formato `Producto abierto · {categoría}`. Por ejemplo: `Producto abierto · Snacks`.
...
Si la captura es válida, agrega una línea al carrito:

```text
1 × Producto abierto · Snack                              $45.00
    Excepción · no afecta inventario
```
...
- No hay PIN de Manager por cada línea de monto abierto.
